...
class SmartController:
    ...
    def _analyze_elements(self, ocr_results: List[Dict], img_shape: Tuple[int]) -> Dict:
        ...
        button_keywords = ['去', '点击', '领取', '提现', '签到', '预约', '打卡', '登录', '看广告','看视频赚金币']
...
if __name__ == "__main__":
    ...
    # 获取所有可点击元素
    elements = controller.detect_clickable_elements("temp_screen_0.png")
    print("检测到的按钮：")
    for btn in elements['buttons']:
        print(f"- {btn['text']} (中心位置: {btn['center']})")

    # 可视化结果
    controller.visualize_results("temp_screen_0.png", "output.png")