        ...
        while time.time() - start_time < timeout:
            try:
                ...
                if elements:
                    ...
                    if element and any(t in elements[0].text for t in ["奖励已发放", "我要加速领奖", "摇动手机", "奖励将于"]):
                        if elements[0].text == "奖励已发放":
                            self.log("✅ 发现奖励已发放")
                            click_by_xpath_text(self.d, "奖励已发放")
                        elif elements[0].text == "我要加速领奖":
                            self.log("✅ 发现加速弹框")
                            click_by_xpath_text(self.d, xpaths="//*[contains(@text, 我要加速领奖)]/../../../following-sibling::*[2]//android.widget.Image")
                ...
                if self.d(textContains="反馈").exists:
                    self.log("🗨️ 发现反馈弹窗，点击取消")
                    if self.d(textContains="跳过").wait_gone(timeout=35):
                        click_by_xpath_text(self.d, xpaths="//*[contains(@text, '反馈')]/../following-sibling::*[1]")
                ...
                if self.d.xpath('//*[contains(@text, "查看福利")]').exists:
                    self.log_debug("🗨️ 发现-查看福利-弹窗")
